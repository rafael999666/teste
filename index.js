...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Salve doidaum @${num.split('@')[0]}\ Bem vindo ao grupo🤡🤙 *${mdata.subject}* aproveita e mama os ADM 😔✊`
          ...
          case 'ping':
            ...
            case 'kangcopet':
                ...
                case 'wolflogo':
                      ...
                      break
                case 'hentai2':
client.sendMessage(from, {url: https://www.google.com/amp/s/pt.amp.xhamster.com/photos/gallery/random-hentai-gifs-9805925, image, {quoted: mek})
break
                case 'truth':
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Parabéns🥳 por se tornar um administrador desse grupo(+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Parabéns🥳 @${mentioned[0].split('@')[0]} por se tornar um administrador desse grupo(+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Tomar no cu Ghost Fdp😡🇬🇧 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Tomar no cu, Ghost Fdp @${mentioned[0].split('@')[0]} 😡🇬🇧`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
					if (!isGroup) return reply(mess.only.group)
					teks = `Todos os ADMs deste grupo *${groupMetadata.subject}*\n𝗧𝗼𝘁𝗮𝗹 : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('❬ SUCESSO ❭ 𝗠𝗲𝗻𝗼𝗻𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 𝗳𝗶𝘁𝘂𝗿 𝘄𝗲𝗹𝗰𝗼𝗺𝗲/𝗹𝗲𝗳𝘁 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽 𝗶𝗻𝗶️')
					} else {
